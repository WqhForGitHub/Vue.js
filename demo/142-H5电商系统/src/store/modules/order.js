export default {
  ...
  mutations: {
    ...
    ADD_ORDER(state, order) { state.list.unshift({ ...order, id: Date.now(), status: 'pending' }) },
    ...
    fetchOrders({ commit }) {
        return new Promise(resolve => {
          setTimeout(() => {
            ...
            commit('SET_LIST', list); resolve()
          }, 300)
        })
      },
    createOrder({ commit }, order) { commit('ADD_ORDER', order) },
    updateOrderStatus({ commit }, payload) { commit('UPDATE_STATUS', payload) },
    filterByStatus({ commit }, status) { commit('SET_STATUS', status) }
  }
}
