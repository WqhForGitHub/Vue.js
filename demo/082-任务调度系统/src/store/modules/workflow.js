export default {
  ...
  mutations: {
    ADD_FLOW(state, flow) { state.flows.push({ ...flow, id: Date.now() }) },
    START_INSTANCE(state, flowId) {
        const flow = state.flows.find(f => f.id === flowId)
        if (flow) {
          state.instances.push({
            id: Date.now(), flowId, flowName: flow.name,
  ...
  actions: {
    addFlow({ commit }, flow) { commit('ADD_FLOW', flow) },
    startInstance({ commit }, flowId) { commit('START_INSTANCE', flowId) },
    advanceStep({ commit }, instanceId) { commit('ADVANCE_STEP', instanceId) }
  }
}
