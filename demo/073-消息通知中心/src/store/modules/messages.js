export default {
  ...
  mutations: {
    SET_CURRENT(state, session) { state.currentSession = session },
    SEND_MESSAGE(state, { sessionId, text }) {
        ...
        if (session) { session.lastMessage = text; session.unread = 0 }
      },
  ...
  actions: {
    selectSession({ commit }, session) { commit('SET_CURRENT', session); commit('MARK_READ', session.id) },
    sendMessage({ commit }, payload) { commit('SEND_MESSAGE', payload) }
  }
}
