export default {
  ...
  mutations: {
    ADD_TASK(state, task) { state.list.push({ ...task, id: Date.now(), status: 'todo' }) },
    UPDATE_STATUS(state, { id, status }) {
        const task = state.list.find(t => t.id === id)
        if (task) task.status = status
      },
    DELETE_TASK(state, id) { state.list = state.list.filter(t => t.id !== id) },
    SET_FILTER(state, filter) { state.filter = filter }
  },
  actions: {
    addTask({ commit }, task) { commit('ADD_TASK', task) },
    updateStatus({ commit }, payload) { commit('UPDATE_STATUS', payload) },
    deleteTask({ commit }, id) { commit('DELETE_TASK', id) },
